...
@app.route('/write_or_add/', methods=["GET", "POST"])
def write_or_add_chapter():
    return render_template('write_or_add_chapter.html', page_title='Welcome')
...
def write():
    if request.method == 'GET':
        ...
        if submit == "New Chapter":
            ...
            return redirect(url_for('newchapter', storyID= newStory, method="POST"))
        else:
            return redirect(url_for('write'))

@app.route('/newchapter/', methods=["GET", "POST"])
def newchapter(storyID=0):
    conn = dbi.connect()
    if request.method == 'GET':
        allStories = webtales.getStories(conn)
        return render_template('newchapter.html', storyID=storyID, title = 'New Chapter', stories = allStories)
    else:
       ...
       # Adding story to database, which returns the chapter_id of the newly added chapter
        ...
        return redirect(url_for('newchapter', storyID=storyID))
